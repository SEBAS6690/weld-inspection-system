...
# ==============================================================================
# Clave privada corporativa exacta requerida por el sistema
...
# ==============================================================================
# FUNCIONES AUXILIARES Y AUTENTICACIÓN
# ==============================================================================
def verify_api_key(x_api_key: str = Header(None), authorization: str = Header(None)):
    """
    Valida que la credencial provista en X-API-Key o Authorization coincida 
    exactamente con la clave corporativa asignada.
    """
    token = x_api_key
...
def convert_cv_to_base64(img_np):
    """
    Convierte una imagen de OpenCV (BGR) a una cadena Base64 lista para renderizar en HTML.
    """
...
async def inspect_weld(
    ...
    Valida la credencial corporativa, procesa la imagen con YOLOv8, 
    aplica tolerancia API 1104 Sec. 9.3 y devuelve la imagen anotada en Base64.
    """
    # 1. Validar autenticación estricta con la clave corporativa
    verify_api_key(x_api_key, authorization)

    try:
        # 2. Leer archivo enviado por la cámara web
        ...
        # 3. Inferencia de Inteligencia Artificial
        results = model(img_bgr, conf=0.25)
        boxes = results[0].boxes

        # Tolerancia máxima admisible según API 1104 Sec. 9.3
        ...
        if len(boxes) > 0:
            # Seleccionar la detección de mayor nivel de confianza
            ...
            # Escalamiento espacial basado en el diámetro exterior de la tubería
            ...
            # Evaluación según criterio API 1104
            ...
            # Dibujar el marco delimitador y la etiqueta sobre la discontinuidad
