...
# Optimización de hilos de CPU para no saturar memoria en Render
...
@app.get("/")
def read_root():
    return {"status": "ONLINE", "system": "Inspección VT API 1104 Libre"}
...
async def inspect_weld(
    ...
    """
    Endpoint de inspección libre sin requerimiento de API Key.
    """
    try:
        contents = await file.read()
        pil_image = Image.open(io.BytesIO(contents)).convert('RGB')
        
        # Reducción preventiva para cuidar la memoria RAM de Render (512MB)
        ...
        # Inferencia con PyTorch sin cálculo de gradientes
        with torch.no_grad():
        ...
        if len(boxes) > 0:
            ...
            if defect_size_mm > max_allowed_mm:
                verdict = "RECHAZADO"
                observations = f"Discontinuidad '{defect_type}' supera la tolerancia máxima por API 1104 ({max_allowed_mm} mm)."
                box_color = (0, 0, 255)
            else:
                verdict = "APROBADO"
                observations = f"Discontinuidad leve '{defect_type}' dentro de la tolerancia de norma."
                box_color = (0, 255, 255)
        ...
        # Limpieza de variables en memoria
